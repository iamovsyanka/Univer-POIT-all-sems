	...
	const WCHAR* szValue)       // smw: const char* szValue
{
	...
	if (szValue != NULL)
	{
		RegSetValueEx(hKey, NULL, 0, REG_SZ, (BYTE*)szValue, 2 * wcslen(szValue) + 1);
	}
...
//DllInstall is used only for application installationand 
//setup.It should not be called by an application.
//It is similar in purpose to DllRegisterServer 
//or DllUnregisterServer.Unlike these functions,
//DllInstall takes an input string which can be used 
//to specify a variety of different actions.This
//allows a DLL to be installed in more than one way,
